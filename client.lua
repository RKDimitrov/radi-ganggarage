...
local Targets, Parking, PlayerGang, Locations = {}, {}, {}, {}
...
local function makeTargets()
	...
	for i = 1, #Locations do
		if Locations[i].garage then
			...
			Targets["GangGarage: "..i] =
				exports['qb-target']:AddBoxZone("GangGarage: "..i, vec3(out.x, out.y, out.z-1.03), 0.8, 0.5, { name="GangGarage: "..i, heading = out.w+180.0, debugPoly=Config.Debug, minZ=out.z-1.05, maxZ=out.z+0.80 },
					{ options = { { event = "radi-ganggarage:client:Garage:Menu", icon = "fas fa-clipboard", label = Loc[Config.Lan].target["label"], gang = Locations[i].gang, spawncoords = Locations[i].garage.spawn, list = Locations[i].garage.list, prop = Parking[#Parking] }, },
					distance = 2.0 })
		end
	end
end

local function syncLocations() local p = promise.new() QBCore.Functions.TriggerCallback('radi-ganggarage:server:syncLocations', function(cb) p:resolve(cb) end) Locations = Citizen.Await(p) makeTargets() end

RegisterNetEvent("radi-ganggarage:client:syncLocations", function(newLocations) Locations = newLocations makeTargets() end)
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()	QBCore.Functions.GetPlayerData(function(PlayerData) PlayerGang = PlayerData.gang end)	Wait(10000) syncLocations() end)
RegisterNetEvent('QBCore:Client:OnGangUpdate', function(GangInfo) PlayerGang = GangInfo end)
AddEventHandler('onResourceStart', function(r) if GetCurrentResourceName() ~= r then return end	QBCore.Functions.GetPlayerData(function(PlayerData) PlayerGang = PlayerData.gang end) Wait(10000) syncLocations() end)

local currentVeh = { out = false, current = nil }
RegisterNetEvent('radi-ganggarage:client:Garage:Menu', function(data)
	...
	if Config.Menu == "qb" then
		vehicleMenu[#vehicleMenu+1] = { icon = "fas fa-car-tunnel", header = PlayerGang.label.." "..Loc[Config.Lan].menu["gangtitle"], isMenuHeader = true }
		vehicleMenu[#vehicleMenu+1] = { icon = "fas fa-circle-xmark", header = "", txt = Loc[Config.Lan].menu["close"], params = { event = "radi-ganggarage:client:Menu:Close" } }
	end
	if currentVeh.out and DoesEntityExist(currentVeh.current) then
		vehicleMenu[#vehicleMenu+1] = {
			...
			event = "radi-ganggarage:client:Garage:Blip",
			params = { event = "radi-ganggarage:client:Garage:Blip", },
		}
		...
		if (Config.distCheck and dist <= 30.0) or not Config.distCheck then
			vehicleMenu[#vehicleMenu+1] = {
				...
				event = "radi-ganggarage:client:RemSpawn",
				params = { event = "radi-ganggarage:client:RemSpawn" }
			}
		end
	else
		...
		for k, v in pairsByKeys(data.list) do
			...
			if showButton == true then
				...
				vehicleMenu[#vehicleMenu+1] = {
					...
					event = "radi-ganggarage:client:SpawnList", args = { spawnName = spawnName, spawncoords = data.spawncoords, list = v },
					params = { event = "radi-ganggarage:client:SpawnList", args = { spawnName = spawnName, spawncoords = data.spawncoords, list = v } }
				}
			end
		end
	end
	...
	if Config.Menu == "ox" then
		exports.ox_lib:registerContext({id = 'vehicleMenu', title = PlayerGang.label.." "..Loc[Config.Lan].menu["gangtitle"], position = 'top-right', options = vehicleMenu })
...
RegisterNetEvent("radi-ganggarage:client:SpawnList", function(data)
	...
	if oldveh ~= 0 then
		...
		QBCore.Functions.SpawnVehicle(data.spawnName, function(veh)
			...
			if data.list.trunkItems then TriggerServerEvent("radi-ganggarage:server:addTrunkItems", QBCore.Functions.GetPlate(veh), data.list.trunkItems) end
...
RegisterNetEvent("radi-ganggarage:client:RemSpawn", function(data)
...
RegisterNetEvent("radi-ganggarage:client:Garage:Blip", function(data)
